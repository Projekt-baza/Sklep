    ...
    <body class="sb-nav-fixed">
      ...
      <!--Logo-->
        ...
        if(isset($_POST['execute'])) {
            ...
            $query='SELECT id_producent, nazwa from producent where id_producent='.$id_producent;
            $st=$pdo->query($query);
            $row=$st->fetch();
            $nazwa=$row['nazwa'];

?>
        <form action="edytproducent.php" method="post">
            <input type="hidden" name="id_producent" value="<?php echo $id_producent; ?>">
...
<?php
    ...
    if(isset($_POST["nazwa"])){
        $query='UPDATE producent SET nazwa="'.
        $_POST["nazwa"].'" WHERE id_producent='.$_POST["id_producent"];
        $st=$pdo->query($query);
