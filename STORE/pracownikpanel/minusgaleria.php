...
<?php
	...
	$query='SELECT id_jpg ,id_produkt, nazwa_zdj from galeria';
	$st=$pdo->query($query);
        if($st == true){
            while($row=$st->fetch()){
                echo "<tr>";
                echo "<td>".$row["id_jpg"]."</td>";
                echo "<td>".$row["id_produkt"]."</td>";
                echo "<td>".$row["nazwa_zdj"]."</td>";
				echo "<td>".'<form action="usungaleria.php" method="post" class="form">
				<input type="hidden" name="execute" value="'. $row['id_jpg'] .'">
