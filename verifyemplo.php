<?php
 ...
 if(isset($_GET["email"]) && isset($_GET["token"])){
     ...
     $sql = "SELECT COUNT(*) AS num FROM pracownik WHERE email = :email and token = :token and potwierdz='0'" ;
     ...
     if($result['num'] == 1){
        $sql = "UPDATE pracownik SET potwierdz='1' where email = :email and token = :token and potwierdz ='0' ";
        $stmt=$pdo->prepare($sql);
        $stmt->bindParam(':email', $email);
        $stmt->bindParam(':token', $token);
        $stmt->execute();
        ...
        '<a href="logowaniecms.php">'.'<button class="btn btn-danger">Zaloguj się</button>'.'</a>'.'<a href=index.php?page=home>'.'<button class="btn btn-warning">Strona Główna</button>'.'</a>'.
